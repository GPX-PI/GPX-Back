...
public class AuthTokenController {
  ...
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "Token refrescado exitosamente", content = @Content(schema = @Schema(example = "{\"accessToken\":\"new-jwt-token\",\"refreshToken\":\"new-refresh-token\",\"message\":\"Token refrescado exitosamente\"}"))),
  ...
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "Logout exitoso", content = @Content(schema = @Schema(example = "{\"message\":\"Logout exitoso\"}"))),
  ...
  public ResponseEntity<?> logout(
    ...
    try {
      ...
      return ResponseEntity.internalServerError()
          .body(Map.of("error", "Error durante logout"));
    }
  }
  ...
  public ResponseEntity<?> logoutAll(@RequestBody Map<String, Object> request) {
    try {
      Object userIdObj = request.get("userId");

      if (userIdObj == null) {
        return ResponseEntity.badRequest()
            .body(Map.of("error", "ID de usuario requerido"));
      }
      ...
      return ResponseEntity.internalServerError()
          .body(Map.of("error", "Error durante logout global"));
    }
  }
  ...
  public ResponseEntity<?> verifyToken(@RequestBody Map<String, String> request) {
    try {
      ...
      return ResponseEntity.internalServerError()
          .body(Map.of("error", "Error verificando token"));
    }
  }
  ...
  public ResponseEntity<?> cleanupTokens() {
    try {
      ...
      return ResponseEntity.ok(Map.of("message", "Limpieza de tokens completada"));

    } catch (Exception e) {
      logger.error("❌ AuthTokenController.cleanupTokens - Error: {}", e.getMessage(), e);
      return ResponseEntity.internalServerError()
          .body(Map.of("error", "Error durante limpieza"));
    }
  }
}